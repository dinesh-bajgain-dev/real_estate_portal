...
import {
  LayoutDashboard, Building2, Users, Plus, Pencil, Trash2,
  Bed, Bath, Maximize, MapPin, Heart, RefreshCw, Search, ShieldCheck
} from 'lucide-react';
import {
  adminFetchStats, adminFetchProperties, adminCreateProperty,
  adminUpdateProperty, adminDeleteProperty, adminFetchUsers, adminDeleteUser,
} from '../api';
...
const formatPrice = (price: number) =>
  price >= 10_000_000 ? `NPR ${(price / 1_000_000).toFixed(1)}M` : `NPR ${(price / 1_000).toFixed(0)}K`;
...
const AdminPage: React.FC = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'property' | 'user'; id: string; name: string } | null>(null);
  ...
  const handleSaveProperty = async (data: PropertyFormData) => {
    try {
      if (modalMode === 'create') {
        ...
        setStats((s) => s ? { ...s, total_properties: s.total_properties + 1 } : s);
    ...
    return (
      <div className="min-h-screen bg-stone-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-stone-400 text-sm font-body">Loading admin panel...</p>
        </div>
      </div>
    );
  }
        ...
        <aside className="w-56 flex-shrink-0 border-r border-stone-800 flex flex-col">
          ...
          <div className="px-5 py-5 border-b border-stone-800">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-display text-white text-base font-semibold">Admin Panel</span>
            </div>
            <p className="text-stone-500 text-xs font-body pl-9">EstatePortal</p>
          </div>
          ...
          {stats && (
            <div className="px-4 pb-5 space-y-2 border-t border-stone-800 pt-4">
              <div className="flex justify-between text-xs font-body">
                ...
                <span className="text-stone-200 font-medium">{stats.total_properties}</span>
              </div>
              <div className="flex justify-between text-xs font-body">
                <span className="text-stone-500">Buyers</span>
                <span className="text-stone-200 font-medium">{stats.total_buyers}</span>
              </div>
              <div className="flex justify-between text-xs font-body">
                <span className="text-stone-500">Favourites</span>
                <span className="text-stone-200 font-medium">{stats.total_favourites}</span>
              </div>
            </div>
          )}
        </aside>
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="sticky top-0 z-10 bg-stone-950 border-b border-stone-800 px-8 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {tab === 'properties' && (
                <button
                  onClick={() => { setModalMode('create'); setEditingProperty(undefined); setShowModal(true); }}
          ...
          <div className="px-8 py-6">
            ...
            {tab === 'overview' && stats && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-3 gap-5">
                  {[
                    { label: 'Total Properties', value: stats.total_properties, icon: Building2, color: 'text-amber-400', bg: 'bg-amber-500/10' },
                    { label: 'Registered Buyers', value: stats.total_buyers, icon: Users, color: 'text-blue-400', bg: 'bg-blue-500/10' },
                    { label: 'Total Favourites', value: stats.total_favourites, icon: Heart, color: 'text-red-400', bg: 'bg-red-500/10' },
                  ].map(({ label, value, icon: Icon, color, bg }) => (
                    <div key={label} className="bg-stone-900 border border-stone-800 rounded-xl p-5">
                      <div className={`w-10 h-10 ${bg} rounded-lg flex items-center justify-center mb-4`}>
                        <Icon size={18} className={color} />
                      </div>
                      <p className="text-stone-400 text-xs font-body mb-1">{label}</p>
                ...
                {/* Recent properties */}
                <div>
                  <h2 className="font-display text-lg text-white mb-4">Recent Listings</h2>
                  <div className="bg-stone-900 border border-stone-800 rounded-xl overflow-hidden">
                    <table className="w-full text-sm font-body">
                      <thead>
                        <tr className="border-b border-stone-800">
                          <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Property</th>
                          <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Type</th>
                          <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Price</th>
                          <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Saved by</th>
                        </tr>
                      </thead>
                      <tbody>
                        {properties.slice(0, 5).map((p) => (
                          <tr key={p.id} className="border-b border-stone-800/50 hover:bg-stone-800/30 transition-colors">
                            ...
                            <td className="px-5 py-3">
                              <span className={`px-2 py-0.5 rounded border text-xs capitalize ${typeColors[p.property_type] || 'bg-stone-700 text-stone-300 border-stone-600'}`}>
                                {p.property_type}
                              </span>
                            </td>
                            <td className="px-5 py-3 text-amber-400 font-mono text-xs">{formatPrice(p.price)}</td>
                            <td className="px-5 py-3 text-stone-400 flex items-center gap-1">
                              <Heart size={11} className="text-red-400" /> {p.favourite_count ?? 0}
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {tab === 'properties' && (
              <div className="space-y-5">
                ...
                <p className="text-stone-500 text-xs font-body">{filteredProperties.length} properties</p>
                ...
                <div className="bg-stone-900 border border-stone-800 rounded-xl overflow-hidden">
                  <table className="w-full text-sm font-body">
                    <thead>
                      <tr className="border-b border-stone-800">
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Property</th>
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs hidden md:table-cell">Details</th>
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Price</th>
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs hidden lg:table-cell">Saved</th>
                        <th className="px-5 py-3 text-stone-400 font-medium text-xs text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredProperties.map((p) => (
                        <tr key={p.id} className="border-b border-stone-800/50 hover:bg-stone-800/30 transition-colors group">
                          ...
                          <td className="px-5 py-4 hidden md:table-cell">
                            <div className="flex items-center gap-3 text-stone-400 text-xs">
                              ...
                              <span className="flex items-center gap-1"><Maximize size={11} />{p.area_sqft.toLocaleString()}</span>
                            </div>
                          </td>
                          <td className="px-5 py-4">
                            <span className="text-amber-400 font-mono text-xs">{formatPrice(p.price)}</span>
                          </td>
                          ...
                          <td className="px-5 py-4">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                onClick={() => { setEditingProperty(p); setModalMode('edit'); setShowModal(true); }}
                              ...
                              <button
                                onClick={() => setDeleteTarget({ type: 'property', id: p.id, name: p.title })}
                  ...
                  {filteredProperties.length === 0 && (
                    <div className="text-center py-16">
                      <Building2 size={32} className="text-stone-700 mx-auto mb-3" />
                      <p className="text-stone-500 text-sm">No properties found.</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {tab === 'users' && (
              <div className="space-y-5">
                ...
                <p className="text-stone-500 text-xs font-body">{filteredUsers.length} users</p>

                <div className="bg-stone-900 border border-stone-800 rounded-xl overflow-hidden">
                  <table className="w-full text-sm font-body">
                    <thead>
                      <tr className="border-b border-stone-800">
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">User</th>
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Role</th>
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs hidden md:table-cell">Joined</th>
                        <th className="text-left px-5 py-3 text-stone-400 font-medium text-xs">Favourites</th>
                        <th className="px-5 py-3 text-stone-400 font-medium text-xs text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredUsers.map((u) => (
                        <tr key={u.id} className="border-b border-stone-800/50 hover:bg-stone-800/30 transition-colors">
                          ...
                          <td className="px-5 py-4">
                            <span className="text-stone-400 text-xs flex items-center gap-1">
                              <Heart size={11} className="text-red-400" /> {u.favourites_count ?? 0}
                            </span>
                          </td>
                          <td className="px-5 py-4">
                            <div className="flex justify-end">
                              {u.role !== 'admin' ? (
                                ...
                                <span className="text-stone-600 text-xs px-3 py-1.5">Protected</span>
                              )}
                            </div>
                          </td>
                        </tr>
